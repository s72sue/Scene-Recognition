...
def process_db(db_name, label_index, opp_index, activations_matrix):
    ...
    for filename in os.listdir(db_name):
        # blurr the image
        image_array = np.array(misc.imread(db_name + '/' + filename))
        blurred_image = ndimage.gaussian_filter(image_array, sigma=1.5)
        img = Image.fromarray(blurred_image.astype(np.uint8), 'RGB')
        new_filename = filename.split('.') + 'blurr.jpg'
        misc.imsave( db_name + new_filename , img)
        
        output = forward_pass(new_filename, db_name)
